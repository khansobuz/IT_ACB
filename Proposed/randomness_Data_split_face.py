"""
IT_ACB_EER.py  — IT-ACB for TJU Palmprint
...
Protocol: Random 8/2 split per subject (fixed seed=42 for reproducibility)
  Train : 8 randomly selected samples per subject
  Test  : 2 remaining samples per subject
  EER   : computed on test samples only

Usage: python IT_ACB_EER.py
"""
...
NUM_SUBJECTS      = 600
NUM_SAMPLES       = 10
TRAIN_PER_SUBJECT = 8       # random 8 samples for training
TEST_PER_SUBJECT  = 2       # remaining 2 samples for test
...
print(f"ArcFace: s={ARC_SCALE}  m={ARC_MARGIN}")


# ============================================================
#  STEP 1: LOAD DATA — random 8/2 split per subject
# ============================================================
def load_tju_features(data_dir):
    train_raw = {}
    test_raw  = {}
    rng = np.random.RandomState(RANDOM_SEED)  # fixed seed = reproducible

    for i in range(1, NUM_SUBJECTS + 1):

        # collect all available sample indices for this subject
        available = []
...
class G_phi(nn.Module):
    ...
    deterministic=False : training  — Z_K = mu + sigma * eps
    deterministic=True  : test time — Z_K = mu  (no noise = better EER)
    """
    def __init__(self):
        super().__init__()
...
class ArcFaceHead(nn.Module):
    """
    Section 3.2: ArcFace angular margin loss.
    Enforces intra-class compactness + inter-class separability.
    Key improvement over CrossEntropy for biometric EER.
    """
...
class CriticNet(nn.Module):
    """Section 3.2: MINE critic T_psi — own optimizer."""
    def __init__(self):
        super().__init__()
...
class CLUBNet(nn.Module):
    """Section 3.2: CLUB network q_xi for cross-key dependency."""
    def __init__(self):
        super().__init__()
...
def mine_critic_loss(critic, z, x):
    """Train critic to maximize MI estimate."""
...
def mine_read(critic, z, x):
    """Read MI estimate — no critic gradient."""
    with torch.no_grad():
        idx = torch.randperm(x.size(0), device=x.device)
        Tj  = critic(z.detach(), x)
        Tm  = critic(z.detach(), x[idx])
...
def club_loss(club, z_i, z_j, y_oh):
    """CLUB upper bound for I(Z_Ki; Z_Kj | Y)."""
...
def sep_loss_fn(mu1, s1, mu2, s2):
    """Section 3.1: L_sep hinge loss."""
    s1 = torch.clamp(s1, 1e-4)
    s2 = torch.clamp(s2, 1e-4)
...
def adv_inner_loss(adv, z, x, key):
    """Adversary inner step: reconstruct with wrong key."""
    B  = z.size(0)
...
def adv_outer_loss_for_generator(adv, z, x, key):
    """Generator outer: minimize adversary best-case similarity."""
    B        = z.size(0)
...
# ============================================================
#  EXTRACT TEMPLATES — test set only, L2-norm mu, no noise
# ============================================================
def extract(g_phi, test_raw, fixed_key=0):
    ...
    with torch.no_grad():
        for (s, j), feat in test_raw.items():
            ...
            # deterministic=True: mu only, no noise
            mu, _, _ = g_phi(x, key, deterministic=True)
            # L2 normalize — consistent with ArcFace training space
...
# ============================================================
#  EER — computed on test templates only
# ============================================================
def cos_sim(a, b):
...
def genuine_scores(prot):
    """Genuine: different samples of same subject."""
    g = []
    for i in range(1, NUM_SUBJECTS+1):
...
def impostor_scores(prot):
    """Impostor: same sample index, different subjects."""
    imp = []
    for idx, (s1, s2) in enumerate(
            combinations(range(1, NUM_SUBJECTS+1), 2)):
...
if __name__ == "__main__":
    ...
    print("\n=== Step 2: Train IT-ACB (train samples only) ===")
