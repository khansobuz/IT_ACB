...
#  SETTINGS
# ============================================================
DATA_DIR      = r"C:\Users\khanm\Desktop\UESTC\PhD. Paper\5th_paper_IEEE_TIFS\Feauture_extraction\WebFace_clean_3000"
NUM_SUBJECTS  = 2000
NUM_SAMPLES   = 10
TRAIN_SAMPLES = [1, 2, 3, 4, 5, 6, 7]
TEST_SAMPLES  = [8, 9, 10]
FEATURE_DIM   = 512
PROTECT_DIM   = 512
NUM_KEYS      = 10
...
# FINAL security weights (reached after warmup) — slightly lower than v1
DELTA_PER_DIM   = 0.3
...
# WARMUP: security weights ramp from 0 -> FINAL over these many epochs
# Epochs 1..WARMUP_EPOCHS: model focuses on learning identity (L_rec) first
WARMUP_EPOCHS   = 15
...
print(f"Epochs: {EPOCHS}  |  Warmup epochs: {WARMUP_EPOCHS}  |  Batch: {BATCH_SIZE}")


# ============================================================
#  LOAD DATA
# ============================================================
def load_webface_features(data_dir):
    train_raw = {}
    test_raw  = {}
...
class G_phi(nn.Module):
    ...
    IMPROVEMENT: residual connection — mu = x_proj + mu_head(h)
    This means the generator only needs to learn a PERTURBATION on top
    of the original identity direction, not reconstruct it from scratch.
    Makes it much easier to preserve recognition accuracy.
    """
    def __init__(self):
        super().__init__()
        ...
        # Residual projection: identity mapping from input x to output space
        # (since FEATURE_DIM == PROTECT_DIM here, this can be identity,
        #  but we keep a learnable linear layer for flexibility)
        self.residual_proj = nn.Linear(FEATURE_DIM, PROTECT_DIM)

        self.apply(_init)
        # Initialize residual_proj close to identity mapping
        with torch.no_grad():
    ...
    def forward(self, x, key, deterministic=False):
        ...
        residual = self.residual_proj(x)          # identity-preserving path
        mu       = residual + self.mu_head(h)     # KEY FIX: residual connection
...
def impostor_scores_fast(prot, max_subjects=500):
    imp = []
    subj_list = list(range(1, min(NUM_SUBJECTS+1, max_subjects+1)))
...
def evaluate_test_eer(g_phi, test_raw, fixed_key=0, max_subjects=500):
    g_phi.eval()
    prot = {}
    with torch.no_grad():
    ...
    imp = impostor_scores_fast(prot, max_subjects=max_subjects)
...
def get_warmup_weight(epoch, warmup_epochs, final_weight):
    """
    Linear warmup: weight ramps from 0 to final_weight over warmup_epochs.
    After warmup_epochs, stays at final_weight.
    """
...
# ============================================================
#  TRAINING — with warmup schedule + residual G_phi
# ============================================================
def train(train_raw, test_raw):
    print("\n=== Training IT-ACB v2 (residual + warmup) ===")
    ...
    # Cosine annealing — smoother than step decay
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # ---- Warmup schedule for security loss weights ----
        ...
        for x, y, key in loader:
            ...
            # KEY FIX: warmed-up weights instead of fixed from epoch 1
        ...
        if epoch % 5 == 0 or epoch == 1:
            ...
            if epoch % EVAL_EVERY == 0 or epoch == 1:
                eer = evaluate_test_eer(g_phi, test_raw, max_subjects=500)
                if eer is not None:
...
def compute_eer_full(gen, imp):
    ...
    print("\n-------------------------------------")
    print("      Performance Result (IT-ACB v2, Face)")
    print("-------------------------------------")
...
def plot_roc(FAR, GAR, eer):
    ...
    plt.plot(FAR*100, GAR*100, 'b-', lw=2, label='IT-ACB v2 (Face)')
    plt.scatter([eer*100],[(1-eer)*100], c='red', zorder=5,
                label=f'EER={eer*100:.2f}%')
    plt.xlabel('FAR (%)'); plt.ylabel('GAR (%)')
    plt.title('ROC — CASIA-WebFace (IT-ACB v2)')
    plt.legend(); plt.grid(True); plt.tight_layout()
    plt.savefig('roc_IT_ACB_webface_v2.png', dpi=150)
    print("ROC saved: roc_IT_ACB_webface_v2.png")
...
if __name__ == "__main__":
    print("=== Step 1: Load CASIA-WebFace clean (train/test split) ===")
    train_raw, test_raw = load_webface_features(DATA_DIR)

    print("\n=== Step 2: Train IT-ACB v2 (residual + warmup) ===")
    ...
    torch.save(g_phi.state_dict(), 'g_phi_webface_v2.pth')
    print("Model saved: g_phi_webface_v2.pth")